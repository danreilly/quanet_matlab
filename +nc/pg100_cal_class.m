classdef pg100_cal_class < nc.pg100_class
% extends pg100 class with calibration functions (NuCrypt internal use)
  ...
  methods
    ...
    % CONSTRUCTOR
    function me = pg100_cal_class(arg1, arg2)
    % desc: pg100 constructor
      import nc.*
      args{1}=arg1;
      if (nargin==1)
        arg2.dbg=0;
      end
      me = me@nc.pg100_class(arg1, arg2);
    end
    
    function set_password(me, pwd)
      me.pwd = pwd;
      fprintf('pg100_cal_class dbg: SET PASSWORD\n');
    end
    
    function get_settings(me)
      me.get_settings@nc.pg100_class(); % superclass get settings
   ...
   function set_atten_dac(me, chan, dac)
     rsp = me.ser.do_cmd(sprintf('cfg cal andvr w %d %d\r', chan, dac));
     me.settings.andvr_dac(chan)=dac;
   end

   function set_andvr_atten_spline(me, ch, fname, m)
   % inputs:
