classdef intf1000_cal_class < nc.intf1000_class
% extends intf1000 class with calibration functions (NuCrypt internal use)
  ...
  methods
    ...
    % CONSTRUCTOR
    function me = intf1000_cal_class(arg1, arg2)
    % desc: intf1000 constructor
      import nc.*
      args{1}=arg1;
      if (nargin==1)
        arg2.dbg=0;
      end
      me = me@nc.intf1000_class(arg1, arg2);
    end
    
    function set_password(me, pwd)
      me.pwd = pwd;
      % fprintf('intf1000_cal_class dbg: SET PASSWORD\n');
    end
    
    function get_settings(me)
      me.get_settings@nc.intf1000_class(); % superclass get settings
    ...
    function [rsp err]=protected_cmd(me, cmd)
    ...
    % proper operation.
      ...
      rsp=me.ser.do_cmd(cmd); % config menu
    ...
    function err=set_voa_dB2dac_spline(me, chan, fname, spline)
      err = set_spline(me, 'm',  chan, fname, spline);
    end
    function err=set_lsr_adc2mW_spline(me, chan, fname, spline)
      err = set_spline(me, 'l',  chan, fname, spline);
    end
    
    function err=set_spline(me, cmd, chan, fname, spline)
    % inputs:
    ...
    % on the type of VOA ("bright" or "dark") inside the intf1000.
    % On newer firmware versions, this function is typically only used during calibration.
      ...
      elseif (nc.util.ver_is_gte(me.devinfo.fwver, [6 2]))
        ...
        % Note: intf 6.0.1 has "2" cmd "set optical pwr",
        %       but intf 4.0 has "2" cmd to "adjust pulse"
    ...
    function set_waveplate_dac(me, pc, wp, val, pwd)
     %inputs: wp:1..6
    % introduced for intf fwv 7.2
      if (nargin<5)
        pwd = me.settings.password;
      end
      if ((wp<1)||(wp>me.devinfo.num_wp(pc)))
        error(sprintf('intf1000_class.cal_set_waveplates_dac: waveplate %d of pc %d nonexistant', wp, pc));
      end
    ...
    %       and the straight line fitted to this model
      if (nc.util.ver_is_gte(me.devinfo.fwver, [7 3 5]))
        if ((ch<1)||(ch>2))
          error(sprintf('intf1000_class.cal_set_delayer_coarse_cal(%d): bad ch', ch));
        end
        if (me.goto_cal_menu())
          error('intf1000_class.cal_set_delayer_coarse_cal(): cant get to cal menu');
        end
        if (length(m)~=12)
          error('intf1000_class.cal_set_delayer_coarse_cal(m): m must be length 12');
        end
        me.ser.do_cmd('C');
        ...
        fprintf('ERR: intf1000_class.cal_set_delayer_coarse(%d)\n', ch);
    ...
    function set_delayer_coarse_dac(me, ch, coarse_dac)
	  ...
	  return;
        ...
        fprintf('ERR: intf1000_class.set_delayer_coare_dac(%g)\n', coarse_dac);
    ...
    % tune_dac: fine tune setting in dac units
	  ...
	  return;
        ...
        fprintf('ERR: intf1000_class.set_delayer_tune_dac(%g)\n', ph_ps);
